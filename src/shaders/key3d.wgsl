...
fn vs_key(v: VsIn, inst: Instance) -> VsOut {
    ...
    // Press: pivot behind the visible key so both edges tilt,
    // but the bottom (viewer-facing) edge dips ~4x more than the top.
    let angle = press * 0.025;
    let cos_a = cos(angle);
    let sin_a = sin(angle);
    let z_pivot = -key_d * 0.3; // virtual pivot behind top edge
    let dz = p.z - z_pivot;
    p = vec3<f32>(p.x, p.y * cos_a - dz * sin_a, p.y * sin_a + dz * cos_a + z_pivot);
